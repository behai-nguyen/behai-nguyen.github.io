---
layout: post
title: "Rust: Multilingual PDFs — an Introductory Study"

description: A “multilingual” PDF simply means a PDF document that supports displaying, copying, and pasting text in any Unicode-supported language. I’ve used several third-party libraries to generate multilingual PDF documents in previous jobs. I was under the impression that as long as we used the correct fonts, the text would render automatically—and so would copy and paste. But when I tried to create a multilingual PDF with Rust, I soon realized it’s not that simple! With help from ChatGPT and Copilot, we discovered that quite a bit of work is needed to get everything working. It’s not as simple as just embedding a font program in the PDF document. In this article, we’ll take an introductory look at creating a multilingual PDF document that includes both Chinese and Vietnamese text. The Rust code we’re writing runs on both Windows and Ubuntu.
...
*A <em>“multilingual”</em> PDF simply means a <strong>PDF document that supports displaying, copying, and pasting text in any Unicode-supported language</strong>.*
...
❶ <strong>The <a href="https://crates.io/crates/lopdf" title="lopdf: A Rust library 
for PDF document manipulation." target="_blank"><code>lopdf</code></a> Crate and Some 
Referenced Documentation</strong>

<a id="lopdf-crate"></a>
⓵ After studying several crates, I decided on 
...
<li style="margin-top:10px;">
Best Use Case — PDF post-processing: for users who want full control over PDF 
internals (merging, splitting, editing metadata, repairing malformed files, etc.).
</li>
</ul>

<a id="ref-docs"></a>
⓶ Based on the <a href="https://docs.rs/lopdf/0.38.0/lopdf/" title="Crate lopdf" 
...
The <code>PDF 1.7 Reference Document</code> is a punishing read... I’ve gone over 
chapter <strong>9 Text</strong> several times. The rest I haven’t covered. 
In addition to this reference, Adobe has published perhaps thousands of 
technical documents. I’ve read the following two:
...
target="_blank">Adobe Technical Note #5014, Adobe CMap and CIDFont Files Specification</a> — 
...
target="_blank">Adobe Technical Note #5411, ToUnicode Mapping File Tutorial</a> — 
Explains how copy and paste works.
</li>
</ol>

I’ve also read the following six articles by Mr. Jay Berkenbilt, which I found very helpful. 
...
❷ <strong>The <a href="https://crates.io/crates/ttf-parser" title="ttf-parser: A 
...
This crate seems to be very popular—at the time of writing this article, it already 
...
target="_blank"><code>pub fn units_per_em(&self) -&gt; u16</code></a> — This <code>u16</code> 
...
— Loosely speaking, this <code>GlyphId</code> is the font program’s 
...
🙏 Can you see that the contents of <code>glyphs_for_text</code> match the output of 
...
🪟 <code>hb-shape</code></a> that we discussed in the second article?

<a id="win-pdfxplorer"></a>
<p>
❸ <strong>The <code>PDFXplorer</code> Windows GUI Application</strong>
</p>

<p>
This application enables us to inspect the internal structure of PDF documents. 
It also allows us to export certain data. It’s a very powerful tool—at least in my opinion.
</p>
...
I’d like to keep a log of progress exactly as it took place. Future code may 
...
💡 As mentioned at <a href="#required-harfbuzz">the outset</a>, this code requires 
the <code>HarfBuzz</code> text shaping library—it makes FFI calls into this library.
...
🦀 Again, to keep things simple, we use absolute paths for everything in the code— 
mostly font program files. Chances are you’ll need to modify the code to work 
with your system.
...
title="Rust FFI Font Subsetting Using the HarfBuzz Text Shaping Engine" 
target="_blank">the conclusion</a> of the second article, we mentioned that we’d turn 
the code in the 
...
module: A helper module. It’s only 81 lines including comments and should be 
self-explanatory.
...
only text. Each page also has a vector of <code>u8</code>, representing the text’s 
...
module: Responsible for generating the PDF document. The call sequence is listed below — 
...
● Function <a href="https://github.com/behai-nguyen/polyglot_pdf/blob/main/pdf_01/src/pdf_gen.rs#L17-L83" 
...
as noted in the inline comments. It’s a work in progress.
...
<code>prepare_page_content()</code></a>, we take the page’s 
...
● The <code>Identity-H</code> encoding: Specifies a direct 1:1 mapping between 
...
re-encoding—just use the embedded font’s glyphs as-is. When using <code>Identity-H</code>, 
text data is typically written as big-endian <code>u16</code> values to the 
<code>Tj</code> or <code>TJ</code> operators.

● <code>/Registry (Adobe) /Ordering (Identity) /Supplement 0</code>: This defines
the <em>character collection</em> for the CIDFont. The "Identity" collection means
no external mapping is used — glyph selection is entirely determined by the CIDs
...
reader: “This is a Unicode-style CIDFont where the character codes map directly to
glyph IDs inside the embedded font.”
...
❺ <strong>Examine Generated PDFs with <code>PDFXplorer</code></strong>

<a id="pdfxplorer-ubuntu-pdf-font"></a>
Let’s take another look at the PDF generated on 🐧 Ubuntu using <code>PDFXplorer</code>. 
...
The screenshot below shows page 1 — the <code>/Kids[0]</code> object:
...
represents the <code>glyph bytes</code> — big-endian <code>u16</code> byte encoding of the text 
<code>"Kỷ độ Long Tuyền đới nguyệt ma."</code>.
...
❻ <strong>Examine Generated PDFs with <code>pdffonts</code></strong>
...
title="Rust FFI “Adventure” with the HarfBuzz Text Shaping Engine" 
target="_blank">🐧 Ubuntu installation</a> we performed in the first article also installs 
...
We can see that there’s no issue with the font, and the font name matches the one shown by 
<a href="#pdfxplorer-ubuntu-pdf-font"><code>PDFXplorer</code></a> — 
...
practical value beyond illustrating how to create a multilingual PDF. There’s still 
a lot to do.

Although I’ve used the <a href="https://crates.io/crates/lopdf" title="lopdf: A Rust library 
for PDF document manipulation." target="_blank"><code>lopdf</code></a> crate in this 
article, I’m still not very familiar with it. I’ll need to spend more time studying 
this crate.

I have a lot of ideas to explore. I’m not yet sure what the next article in this series will be. 
As I continue to explore this subject, I’ll document it whenever there’s something worthwhile to share.

Thanks for reading! I hope this post helps others who are looking to deepen their understanding 
of PDF technology. As always—stay curious, stay safe 🦊
